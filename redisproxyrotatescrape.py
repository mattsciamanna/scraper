...
# \\\\\\\\\\\\\\\\\
# test link where seller has a limit per customer
...
UNAVAILABLE = "SORRY, THIS GUY IS NOT AVAILABLE"
SELLERLIMIT = "SELLER HAS PLACED A LIMIT ON THIS"
OTHER = "OTHER ISSUE ENCOUNTERED"


myclient = pymongo.MongoClient("mongodb://localhost:27017/")
mydb = myclient["mydatabase"]
mycol = mydb["ScrapeResults"]
othercol = mydb["Messedup"]
...
def failed_product(item):
    ...
    r.rpush('queue:failedproducts', json.dumps(data))
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                print "hopefully she's just out of the queue"
                print "we've reached the end of the queue or been out of internet too long"
                print "we've reached the end of the queue or been out!"
                print "we've reached the end of the queue or else its messed up!!"
                break
        ...
        if tags != []:
            ...
            try:
                ...
                try:
                    mydict = { "Asin": item, "Average Review": avgratings[item], "Number of Reviews": numReviews, "Inventory Levels": quantnospace[0], "Date": datetime.datetime.fromtimestamp(time.time()).strftime('%m%d'), "Other Notes": "firstpage quantity" }
                    x = mycol.insert_one(mydict)
                except:
                    print "db insertion didn't work"
                    try:
                        messedupdict = { "Asin": item, "Note": 'tried on line 436 but failed.'}
                        x = othercol.insert_one(messedupdict)
                    except: 
                        pass
                    pass
        ...
        try:
            ...
            try:
                    mydict = { "Asin": item, "Average Review": avgratings[item], "Number of Reviews": numReviews, "Inventory Levels": number, "Date": datetime.datetime.fromtimestamp(time.time()).strftime('%m%d'), "Other Notes": "Success" }
                    x = mycol.insert_one(mydict)
            except:
                    print "db insertion didn't work"
                    try:
                        messedupdict = { "Asin": item, "Note": 'tried on line 545 but failed.'}
                        x = othercol.insert_one(messedupdict)
                    except: 
                        pass
                    pass
